...
<body>
  ...
  {% else %}
      ...
      <div class="controls">
        <button id="startCamBtn">Kamera Yayını Başlat</button>
        <button id="startScreenBtn">Ekran Yayını Başlat</button>
...
<script>
    ...
    async function startStream(isScreen) {
        if (!socket.connected) {
             statusDiv.textContent = 'Socket bağlantısı yok. Sayfayı yenileyin.';
             return;
        }

        statusDiv.textContent = 'Kamera/Ekran erişimi bekleniyor...';
        try {
            if (isScreen) {
                // Ekran paylaşımı
            ...
            statusDiv.textContent = 'Yayın başlatılamadı. İzinleri kontrol edin.';
        }
    }
    ...
    if (startCamBtn) startCamBtn.onclick = () => startStream(false);
    if (startScreenBtn) startScreenBtn.onclick = () => startStream(true);
